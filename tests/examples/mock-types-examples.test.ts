...
describe('mock Types Examples', () => {
  ...
  describe('1. vi.mocked() - 推荐方式', () => {
    it('should mock with full type safety', () => {
      ...
      // 创建类型化的 mock
      const mockFs = vi.mocked(mockModule)
    ...
    it('should mock complex return types', () => {
      ...
      // Mock 返回复杂的对象
      mockFs.statSync.mockImplementation((path): FileStats => {
  ...
  describe('3. vi.spyOn() 进行部分 Mock', () => {
    it('should spy on specific methods', () => {
      ...
      const mockReadFileSync = vi.spyOn(fsModule, 'readFileSync')
      ...
      // 验证
      expect(fsModule.existsSync('/test/file.txt')).toBe(true)
      expect(fsModule.readFileSync('/test/file.txt')).toBe('file content')
